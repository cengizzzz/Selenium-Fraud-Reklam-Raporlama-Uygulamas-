...
namespace AdClicker
{
    ...
    public partial class Form1 : Form
    {
        ...
        public void sendMailAsync(string url)
        {
            try
            {
                MailMessage message = new MailMessage();
                SmtpClient smtp = new SmtpClient();
                message.From = new MailAddress(userMailAddress.Text);
                message.To.Add(new MailAddress(targetMailAddress.Text));
                message.Subject = "Oltalama Şikayeti";
                message.IsBodyHtml = true; 
                message.Body = reportContent.Text.Replace("{URL}", url);
                smtp.Port = 587;
                smtp.Host = "smtp.gmail.com";
                smtp.EnableSsl = true;
                smtp.UseDefaultCredentials = false;
                smtp.Credentials = new NetworkCredential(userMailAddress.Text, userPassword.Text);
        ...
        private void worker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < urls.Count; i++)
            {
                ...
                string address;
                if (driver.Url.Contains("signin/v2/identifier"))
                {
                    address = driver.Url.Split(new string[] { "continue=" }, StringSplitOptions.None)[1].Replace("%3A", ":").Replace("%2F", "/");
                } else
                {
                    address = driver.Url;
                }
                sendMailAsync(address);
